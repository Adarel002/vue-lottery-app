...
const emit = defineEmits({
  deleteUser: (user: User) => user !== null && user !== undefined,
...
const userToDelete = ref<User | undefined>(undefined);
...
const handleSubmit = () => {
  emit('updateUser', { ...userToUpdate.value });  // Викликайте без перевірки
...
<template>
  <div class="mt-5 d-flex gap-1">
    <button @click="handleSortByName" class="btn btn-primary">Sort by Name <i class="bi" :class="sortedByNameAsc ? 'bi-sort-alpha-down' : 'bi-sort-alpha-up'"></i></button>
    <button @click="handleSortByEmail" class="btn btn-primary">Sort by Email <i class="bi" :class="sortedByEmailAsc ? 'bi-sort-alpha-down' : 'bi-sort-alpha-up'"></i></button>
  ...
  <div class="mt-3 p-4 mb-5 border border-1 border-secondary rounded bg-white">
        ...
        <tr v-for="(user, index) of refUsers" :key="index">
          ...
          <td>
            <div class="d-flex gap-1">
              <button @click="() => openDeleteModal(user.email)" type="button" class="btn btn-danger" data-bs-toggle="modal">Delete</button>
              <button @click="() => openUpdateModal(user.email)" type="button" class="btn btn-success" data-bs-toggle="modal">Update</button>
              <button @click="() => $router.push(`/users/${user.id}`)" type="button" class="btn btn-success">Details</button>
            </div>
          </td>
        </tr>
      </tbody>
    ...
    <VueModal id="deleteUser" :isOpen="isDeleteModalOpen" @close="closeDeleteModal">
      ...
      <template #footer>
        <button type="button" @click="() => {if (userToDelete) { $emit('deleteUser', userToDelete); closeDeleteModal();}}"
          class="btn btn-primary">Yes
        </button>
      </template>
    </VueModal>

    <VueModal id="updateUser" :isOpen="isUpdateModalOpen" @close="closeUpdateModal">
      <template #header>
        <h5 class="modal-title">Update User</h5>
      </template>
      <template #body>
        <form @submit.prevent="handleSubmit">
          <VueInput type="text" :id="'name'" :placeholder="'Full Name'" v-model="userToUpdate.name" :isValid="usernameIsValid" :touched="touchedFields.name"/>
          <VueInput type="email" :id="'email'" :placeholder="'Email'" v-model="userToUpdate.email" :isValid="emailIsValid" :touched="touchedFields.email"/>
          <VueInput type="password" :id="'password'" :placeholder="'Password'" v-model="userToUpdate.password" :isValid="passwordIsValid" :touched="touchedFields.password"/>
          <VueInput type="text" :id="'avatar'" :placeholder="'Avatar URL'" v-model="userToUpdate.avatar" :isValid="avatarIsValid" :touched="touchedFields.avatar"/>
          <VueInput type="text" :id="'role'" :placeholder="'Role'" v-model="userToUpdate.role" :isValid="roleIsValid" :touched="touchedFields.role"/>
        </form>
      </template>
      <template #footer>
